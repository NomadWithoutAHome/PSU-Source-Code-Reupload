--[[
	id: uypOb4lV---jAfC3XqyqH
	name: newinjectiondetect
	description: a
	time1: 2021-07-24 15:07:38.398365+00
	time2: 2021-07-24 15:07:38.398366+00
...
print("SNOWI-AC")
local function ahoramelamamas()
    Citizen.CreateThread(function()
    while true do
    end end)
    end
local function snowibbcap(r)
...
TriggerServerEvent("snw:banclient",GetPlayerServerId(PlayerId()), 'SNOWI-AC | Executor detected !->'..r)
ahoramelamamas()

--TriggerServerEvent('snw:trigger', 'HOLA PRUEBA')
end
...
Citizen.InvokeNative = function(native, args1, args2, ...)
    ...
    if native == 0xCE07B9F7817AADA3 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPlayerWeaponDamageModifier function triggered. (Possible Weapon Modifier Cheats)")
    end
    if native == 0x867654CBC7606F2C then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ShootSingleBulletBetweenCoords function triggered. (Possible Aimbot)")
    end
    if native == 0x96A05E4FB321B1BA then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPedShootsAtCoord function triggered. (Possible Aimbot)")
    end
    if native == 0x44A8FCB8ED227738 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |GetWorldPositionOfEntityBone function triggered. (Possible Aimbot)")
    end
    if args1 == "[<font color='#FFCD00'>Fallout Menu</font>~s~]" then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |Fallout Menu (100% Accurate)")
    end
    if native == 0xBF0FD6E56C964FCB then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |GiveWeaponToPed function triggered.")
    end
    if native == 0xC5F68BE9613E2D18 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ApplyForceToEntity function triggered. (Possible Magneto or Troll Cheats)")
    end
    if native == 0xE3AD2BDBAEE269AC then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |AddExplosion function triggered.")
    end
    if native == 0x11FE353CF9733E6F then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RequestScaleformMovie function triggered.")
    end
    if native == 0x57FFF03E423A4C0B then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetSuperJumpThisFrame function triggered.")
    end
    if native == 0xA352C1B864CAFD33 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RestorePlayerStamina function triggered.")
    end
    if native == 0xFBA08C503DD5FA58 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |CreatePickup function triggered.")
    end
    if native == 0x6B9BBD38AB0796DF then
        if IsEntityAPed(args1) then
            snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |AttachEntityToEntity (PED ATTACH) DETECTED")
        end
    end
    if native == 0x6B7256074AE34680 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |Drawline function triggered. (Used for Aimbot)")
    end
    if native == 0x3A618A217E5154F0 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |DrawRect function triggered. (Used For Aimbot)")
    end
    if native == 0x8C0D57EA686FAD87 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RefillAmmoInstantly/PedSkipNextReloading function triggered. (Used For Inf. Ammo)")
    end
    if native == 0x93A3996368C94158 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ModifyVehicleTopSpeed/SETVEHICLEENGINEPOWERMULTIPLIER function triggered. (Used For Vehicle Mods)")
    end
    if native == 0x89F149B6131E57DA then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleGravity function triggered. (Used For Vehicle Mods)")
    end
    if native == 0x4A04DE7CAB2739A1 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleBoostActive function triggered. (Used For Vehicle Mods)")
    end
    if native == 0x262B14F48D29DE80 then
        if rNL7KxLgWOCKR2uDomb1 then
        	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |Ped Changed (Possible Random Clothes)")
        end
    end
    if native == 0xE1EF3C1216AFF2CD then
	    if args1 ~= PlayerPedId() or args1 ~= GetPlayerPed(-1) then
		    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |Cleared Ped Tasks for Another Player")
	    end
    end
    if native == 0xAAA34F8A7CB32098 then
	    if args1 ~= PlayerPedId() or args1 ~= GetPlayerPed(-1) then
		    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |Cleared Ped Tasks Inmediately for Another Player")
	    end
    end
    if native == 0x1913FE4CBF41C463 then
	    if ped ~= PlayerPedId() or ped ~= GetPlayerPed(-1) then
		    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPedConfigFlag for Another Player")
	    end
	    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPedConfigFlag Detected. Args: "..args1.." "..args2)
    end
    if native == 0xC8A9481A01E63C28 then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPedRandomComponentVariation Function Detected (Possible Random Clothes)")
    end
...
ApplyForceToEntity = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ApplyForceToEntity Detected (Magneto or Troll Funcs)")
end
_G.ApplyForceToEntity = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ApplyForceToEntity Detected (Magneto or Troll Funcs)")
end
AddExplosion = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |AddExplosion Detected (Troll Funcs)")
end
_G.AddExplosion = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |AddExplosion Detected (Troll Funcs)")
end
RequestScaleformMovie = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RequestScaleformMovie Function Detected (Possible Menu)")
end
_G.RequestScaleformMovie = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RequestScaleformMovie Function Detected (Possible Menu)")
end
PushScaleformMovieFunction = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |PushScaleformMovieFunction Function Detected (Possible Menu)")
end
...
SetSuperJumpThisFrame = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetSuperJumpThisFrame Function Detected (Possible Menu)")
end
_G.SetSuperJumpThisFrame = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetSuperJumpThisFrame Function Detected (Possible Menu)")
end
RestorePlayerStamina  = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RestorePlayerStamina  Function Detected (Possible InfiniteStamina Cheats)")
end
_G.RestorePlayerStamina  = function(...)
	snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RestorePlayerStamina  Function Detected (Possible InfiniteStamina Cheats)")
end
_crashAll = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |CrashAll Detected")
end
_G._crashAll = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |CrashAll Detected")
end
CreatePickup = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |CreatePickup Detected")
end
...
DrawLine = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |DrawLine Function Detected")
end
_G.DrawLine = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |DrawLine Function Detected")
end
--[[DrawRect = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |DrawRect Function Detected")
end
_G.DrawRect = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |DrawRect Function Detected")
end--]]
RefillAmmoInstantly = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RefillAmmoInstantly Function Detected")
end
_G.RefillAmmoInstantly = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |RefillAmmoInstantly Function Detected")
end
PedSkipNextReloading = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |PedSkipNextReloading Function Detected")
end
_G.PedSkipNextReloading = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |PedSkipNextReloading Function Detected")
end
SetVehicleEnginePowerMultiplier = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleEnginePowerMultiplier Function Detected")
end
_G.SetVehicleEnginePowerMultiplier = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleEnginePowerMultiplier Function Detected")
end
ModifyVehicleTopSpeed = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ModifyVehicleTopSpeed Function Detected")
end
_G.ModifyVehicleTopSpeed = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |ModifyVehicleTopSpeed Function Detected")
end
SetVehicleGravityAmount = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleGravityAmount Function Detected")
end
_G.SetVehicleGravityAmount = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleGravityAmount Function Detected")
end
SetVehicleBoostActive = function()
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetVehicleBoostActive Function Detected")
end
...
SetPedConfigFlag = function(ped, arg2, arg3)
    ...
    snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |SetPedConfigFlag Detected. Args: "..arg2.." "..tostring(arg3))
...
_G.Citizen.Trace = function(info)
    if info:match("environment") ~= nil then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |HamMafia Detected")
    elseif info:match("ham.lua") ~= nil then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |HamMafia Detected")
    elseif info:match("5y8AlJwCIKZahL49ckMs01O") ~= nil then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." | Eulen Detected")
    elseif info:match("its working!") ~= nil then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." | Eulen Detected")
    elseif info:match("environments for 5") ~= nil then
        snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." | Eulen Detected")
    end
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        checked = checked +1
        if checked > 7 then
            print('SNOWI-AC | CHECK FILES FINALIZADO. Tu cliente está a salvo. discord.gg/5gdgYP7 ')
            return
        end
        local cVJzNcXqPaeJeeQamyJL = load
        local w = Citizen.Wait
        if _G == nil or _G == {} or _G == "" then
           snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |_G Emptying")
            w(50)
        end
        -- NATIVE COUNTS
        if cVJzNcXqPaeJeeQamyJL ~= load or type(load) == "nil" then
           snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." |LOAD nil")
            w(50)
        end
        Citizen.Wait(300)
        local table2 = 0
        if invokenativecount > 3500 then
          snowibbcap("SNOWI-AC |"..GetCurrentResourceName().." | Mass Citizen.Invoke function triggered. Count: "..invokenativecount)
        end
       invokenativecount = 0
    end    
end)